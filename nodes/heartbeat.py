...
a = DiffieHellman()
a_pubkey = a.gen_public_key()
p_mqtt.single("door/gyro", a_pubkey, qos=1, hostname=HOST, port=PORT)

a_sharedkeyhex = a.gen_shared_key(b_pubkey)
a_sharedkey = int(a_sharedkeyhex,16)
...
while True:
    ...
    hbeat = random.randint(1,99999999)
    print(hbeat)
    
    #if recieved # doesn't equal our #, then interrupted = true FOR NODE
    pub.single("door/gyro", str(hbeat), qos=1, hostname=HOST, port=PORT)
    the_key = sub.simple("door/key", qos=1, hostname = HOST, port = PORT)
